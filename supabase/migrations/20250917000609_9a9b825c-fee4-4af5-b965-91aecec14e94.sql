DROP FUNCTION public.search_addresses_safely(text);

CREATE OR REPLACE FUNCTION public.search_addresses_safely(search_query text)
 RETURNS TABLE(uac text, country text, region text, city text, street text, building text, latitude numeric, longitude numeric, address_type text, description text, verified boolean, public boolean, created_at timestamp with time zone, completeness_score numeric)
 LANGUAGE sql
