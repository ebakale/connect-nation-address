-- Migration: Clean NAR/CAR Separation - Drop All Dependent Policies First
-- This migration separates NAR (infrastructure) from CAR (occupancy) properly
...
-- Step 2: Remove user_id columns (addresses become authority-managed, requests get requester_id)
ALTER TABLE public.addresses DROP COLUMN user_id;
ALTER TABLE public.address_requests DROP COLUMN user_id;

-- Step 3: Add new structure for NAR authority management
...
-- Step 4: Update address_requests for NAR creation workflow
...
-- Step 5: Create NAR authorities table to manage who can create addresses
...
-- Step 6: Create audit log for NAR address creation
...
-- Step 7: Create new RLS policies for NAR-managed addresses
...
-- Step 8: Create RLS policies for NAR creation requests
...
-- Step 9: RLS for NAR authorities
