-- Fix ISO 3166-2:GQ compliance by updating Litoral region code from LT to LI
-- and regenerate UACs for affected addresses

-- First, update the database function to use correct ISO codes
...
BEGIN
  ...
  region_code := CASE p_region
    ...
    WHEN 'Litoral' THEN 'LI'  -- Fixed: LT -> LI per ISO 3166-2:GQ
...
-- Update existing addresses with Litoral region that have incorrect UACs
-- First, let's update addresses where region is 'Litoral' and UAC contains '-LT-'
UPDATE public.addresses 
SET uac = generate_unified_uac_unique(country, region, city, id)
WHERE region = 'Litoral' 
  AND uac LIKE '%-LT-%';

-- Also update any address requests with the same issue
