-- Migration: Separate NAR and CAR - Clean Architecture
-- This migration removes user ownership from addresses and establishes clean separation

-- Step 1: Remove user_id from addresses table (NAR should not be user-owned)
ALTER TABLE public.addresses DROP COLUMN user_id;

-- Step 2: Add authority fields for address creation
...
-- Step 3: Update address_requests to be about NAR address creation, not ownership
...
-- Step 4: Create NAR address authorities table
...
-- Step 5: Create address creation audit log
...
-- Step 6: Update RLS policies for NAR (authority-managed addresses)
DROP POLICY IF EXISTS "Users can view their own addresses" ON public.addresses;
DROP POLICY IF EXISTS "Users can delete their own addresses" ON public.addresses;
DROP POLICY IF EXISTS "Staff can create addresses for users" ON public.addresses;

-- New NAR policies: Addresses managed by authorities
...
-- Step 7: Update address_requests policies for NAR creation requests
...
-- Step 8: Create NAR authorities RLS
...
-- Step 9: Create NAR creation log RLS
...
-- Step 10: Update functions for NAR/CAR separation
