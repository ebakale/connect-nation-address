-- Drop the existing unique constraint temporarily
ALTER TABLE public.addresses DROP CONSTRAINT IF EXISTS addresses_uac_key;
ALTER TABLE public.addresses DROP CONSTRAINT IF EXISTS unique_uac;

-- Create function to generate UAC with truly unique sequences
...
-- Update all addresses to use the new unified UAC format with UUID-based uniqueness
UPDATE public.addresses 
SET uac = public.generate_unified_uac_unique(country, region, city, id)
WHERE uac IS NOT NULL;

-- Add index for better UAC lookup performance
DROP INDEX IF EXISTS idx_addresses_uac;
CREATE INDEX idx_addresses_uac ON public.addresses(uac);

-- Add back the unique constraint
ALTER TABLE public.addresses 
ADD CONSTRAINT unique_uac UNIQUE (uac);