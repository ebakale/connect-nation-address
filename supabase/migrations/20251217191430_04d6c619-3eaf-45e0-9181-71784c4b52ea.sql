-- Drop both versions of the function to clean up duplicates
DROP FUNCTION IF EXISTS public.flag_address_for_review(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.flag_address_for_review(uuid, text, jsonb, text[], uuid);

-- Recreate a single clean version
...
BEGIN
  ...
  -- Update the original address with flag info
