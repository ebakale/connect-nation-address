...
BEGIN
  ...
  -- Generate sequence string with more uniqueness
  sequence_str := LPAD(p_sequence_num::text, 3, '0') || CHR(65 + (p_sequence_num % 26)) || LPAD((EXTRACT(EPOCH FROM NOW())::bigint % 100)::text, 2, '0');
...
-- Create temporary table with unique UACs
