...
const scenarios: DemoScenario[] = [
  ...
  },
  {
    ...
    icon: Truck,
    context: 'The ConEG Postal Delivery Module manages government postal services from order creation to final delivery. This enhanced scenario demonstrates recipient search by name, in-app routing, Cash on Delivery (COD), pickup requests, returns, bulk import, and complete delivery workflow.',
    objective: 'Demonstrate enhanced postal features: recipient name search, in-app routing, COD, pickups, returns, and bulk import',
    actors: [
      ...
      { role: 'Citizen', name: 'Elena Nguema', type: 'secondary' }
    ],
    modules: ['PostalPage', 'DeliveryOrderForm', 'DispatcherPanel', 'AgentDeliveryView', 'SupervisorDashboard', 'PickupRequests', 'ReturnOrders', 'CODManagement', 'BulkImport'],
    steps: [
      { step: 1, action: 'Carmen (Postal Clerk) logs into the Postal Portal', screen: 'PostalPage with Clerk view showing "Create Order" and "Bulk Import" tabs', notes: 'Point out the role-based interface with enhanced features for postal staff.' },
      ...
      { step: 4, action: 'Carmen searches by recipient NAME: "Pablo Martínez Jr." (a household dependent)', screen: 'UACAddressPicker shows name search results including dependents', notes: 'NEW: Recipient search by name finds household members/dependents, not just account holders.' },
      { step: 5, action: 'System shows Pablo\'s address with "Care of: Pedro Martínez" (guardian)', screen: 'Address card with dependent badge and guardian relationship', notes: 'Demonstrate household dependent address resolution for postal delivery.' },
      ...
      { step: 7, action: 'She enables Cash on Delivery (COD) and enters amount: 5,000 XAF', screen: 'CODSection with amount field and payment method options', notes: 'NEW: COD allows payment collection at delivery.' },
      { step: 8, action: 'Carmen generates shipping label with barcode and QR code', screen: 'LabelPreview showing S10 tracking number, barcode, QR code', notes: 'NEW: UPU S10-compliant labels with scannable codes.' },
      { step: 9, action: 'Order submitted with number: DEL-2025-001234 and tracking: EG123456789GQ', screen: 'Success confirmation with order and S10 tracking numbers', notes: 'Note: Order enters dispatcher queue; tracking URL in QR code.' },
      { step: 10, action: 'Luis (Dispatcher) views orders in "Ready for Assignment" tab', screen: 'DispatcherPanel with pending orders list including COD indicators', notes: 'Dispatchers see orders waiting for agent assignment with COD flags.' },
      { step: 11, action: 'He opens "Pickups" tab to see citizen pickup requests', screen: 'PickupRequestsList showing pending pickup requests from citizens', notes: 'NEW: Citizens can request package collection from their addresses.' },
      { step: 12, action: 'Luis schedules a pickup request and assigns to Pedro', screen: 'Pickup scheduling dialog with date/time and agent selection', notes: 'Show pickup request workflow integration.' },
      { step: 13, action: 'He assigns both the delivery and pickup to Pedro García', screen: 'Assignment dialog with route optimization suggestion', notes: 'Demonstrate workload balancing and route efficiency.' },
      { step: 14, action: 'Pedro (Agent) receives notifications on his mobile device', screen: 'Agent mobile view with delivery and pickup assignments', notes: 'Point out mobile-first design for field agents.' },
      { step: 15, action: 'Pedro clicks "Show Route" for embedded in-app navigation', screen: 'RouteMapView with visual route polyline, markers, distance, ETA, turn-by-turn directions', notes: 'NEW: In-app routing using Leaflet/OSRM - no external app needed.' },
      { step: 16, action: 'Route shows: Origin (blue marker) → Destination (red marker), 2.3km, 8 min', screen: 'Interactive map with auto-fitted bounds and scrollable directions', notes: 'Demonstrate embedded navigation keeping agent in app.' },
      { step: 17, action: 'Pedro arrives and clicks "Mark Arrived" - system logs GPS', screen: 'Arrival confirmation with GPS verification', notes: 'Show location verification for delivery accuracy.' },
      { step: 18, action: 'He collects COD payment: 5,000 XAF cash and records receipt', screen: 'COD collection dialog with amount, payment method, receipt number', notes: 'NEW: COD collection tracked with receipt for remittance.' },
      { step: 19, action: 'Pedro captures proof: photo, signature, recipient name', screen: 'Proof capture: photo, signature pad, recipient name confirmation', notes: 'Demonstrate multi-method proof capture for accountability.' },
      { step: 20, action: 'Delivery completed - Elena receives SMS/email notification', screen: 'Customer notification: "Your package has been delivered"', notes: 'NEW: Automated customer notifications at key lifecycle stages.' },
      { step: 21, action: 'Pedro completes the pickup, captures photo proof', screen: 'Pickup completion with proof capture', notes: 'Show pickup request fulfillment workflow.' },
      { step: 22, action: 'Ana (Supervisor) opens "COD Management" tab', screen: 'CODManagement showing pending collections and daily reconciliation', notes: 'NEW: COD tracking from collection to remittance.' },
      { step: 23, action: 'She views agent COD balances and marks Pedro\'s 5,000 XAF as remitted', screen: 'Agent remittance dialog with amount and reference', notes: 'Demonstrate COD reconciliation workflow.' },
      { step: 24, action: 'Ana opens "Returns" tab to manage reverse logistics', screen: 'ReturnOrdersList showing return requests and their status', notes: 'NEW: Returns management for undeliverable or customer-initiated returns.' },
      { step: 25, action: 'Carmen uses "Bulk Import" to upload 50 orders from CSV', screen: 'BulkImportDialog with CSV upload, validation preview, batch processing', notes: 'NEW: Bulk order import with validation and error reporting.' }
    ],
    outcome: [
      'Recipient search by name enables delivery to household dependents without UAC',
      'In-app routing keeps agents in the app with visual navigation and directions',
      'COD collection tracked from payment to supervisor remittance',
      'Pickup requests allow citizens to schedule package collection',
      'Returns/reverse logistics manage undeliverable and customer-returned packages',
      'Bulk import processes high-volume orders efficiently via CSV upload',
      'Automated customer notifications at all delivery lifecycle stages',
      'S10-compliant labels with barcode and QR codes for tracking',
      'Complete audit trail with GPS, photos, signatures, and COD receipts'
    ],
    presenterNotes: 'This enhanced postal scenario showcases all Phase 1-8 features: recipient name search (including household dependents), in-app routing with Leaflet/OSRM (eliminates need for external navigation apps), COD handling from collection to remittance, pickup request service for citizen convenience, returns/reverse logistics for failed deliveries, bulk import for high-volume operations, and automated customer notifications. The integration with the digital address system ensures accurate delivery using UAC-verified addresses.'
  }
];
