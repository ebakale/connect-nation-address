...
export const DeliveryAgentView = () => {
  ...
  const handlePickupStatsUpdate = useCallback((active: number, completed: number) => {
    setPickupStats({ active, completed });
  }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-teal-600">{pickupStats.active}</p>
            <p className="text-xs text-muted-foreground">{t('agent.activePickups')}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="pickups" className="flex items-center gap-1">
            <Truck className="h-3 w-3" />
            <span className="hidden sm:inline">{t('agent.pickups')}</span>
          </TabsTrigger>
        ...
        {['active', 'completed', 'all'].includes(activeTab) && (
        ...
        <TabsContent value="pickups" className="mt-4">
          <AgentPickupsList onStatsUpdate={handlePickupStatsUpdate} />
        </TabsContent>
      </Tabs>
